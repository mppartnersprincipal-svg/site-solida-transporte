...
- **Onde:** `../Assinatura de Email/` (fora do git): `build.py` (gera `out/logo-animada.gif` 380×148 px 2x/141 KB, `logo.png` estática e 5 ícones PNG 32 px a partir de SVG Lucide/Simple Icons renderizados com PyMuPDF; copia tudo + `gerador.html` para `public/assinatura/`), `assinatura.html` (template só com tabelas + estilos inline, Arial, `{{PLACEHOLDERS}}`), `gerador.html` (formulário → prévia → **Copiar assinatura** via ClipboardItem text/html, fallback execCommand; "Baixar .htm"; instruções por cliente), `COMO-INSTALAR.md`. **Regenerar/publicar: `python build.py`**; ao mudar o layout, manter `assinatura.html` e o `TEMPLATE` do `gerador.html` iguais
