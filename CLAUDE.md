...
- **Guia completo em `gtm/TRACKING.md`** (criar propriedade GA4, ações de conversão no Ads, importar container, testar, publicar). Container importável em `gtm/gtm-container-solida.json`, gerado por `gtm/gen-container.mjs` (20 tags, 16 gatilhos, 23 variáveis — **não editar o JSON à mão**, regenerar). Após importar, o usuário preenche 5 constantes `CONST - *` (ID GA4, ID Ads e 3 rótulos de conversão)
...
**Decisões do usuário:** coleta **first-party no Supabase** (GTM/GA4/Ads intocados); coleta **sempre ativa e anônima** (sem cookie, sem IP, sem ID persistente — só `sessionStorage`, renova após 30 min parado); pago × orgânico no site vem de gclid/utm/referrer; `/dashboard` usa o **mesmo login** do `/admin`. A decisão inicial de não consultar custos foi substituída pelo pedido de métricas do Google Ads em 21/09/2026 (seção abaixo). Requisito reforçado: **todo clique com o NOME do botão e de onde veio** (ex.: "Google Ads · campanha X → Home → Central aberta (flutuante) → Pedir cotação › Goiânia") — seções "Botões da Central" e "Jornadas" na D3. Gráficos: Recharts (entra na D2).
...
- Testes da integração: `npm run test:google-ads` (respostas simuladas, sem chamadas à conta real).
- Validação local: build de produção e lint dos arquivos alterados passaram. O lint geral continua apontando erros anteriores de `set-state-in-effect` em `CookieBanner`, `Header` e `WhatsAppModal`.
...
**Novo × recorrente — decisão do usuário (26/08/2026):** identificador persistente aleatório (`localStorage` `solida-v`, 13 meses, sem PII) para **todo visitante, EXCETO quem clicar "Só o essencial"** no banner (esse fica só com a sessão anônima e um id existente é apagado). Base legal: legítimo interesse + respeito à recusa; política de cookies descreve. Também pedido: card de **cobertura de consentimento** (aceitou × só essencial × não respondeu) no dashboard. Ambos entram na D3.
...
### Fase D3 — concluída (26/08/2026)

- **Migration `0004_analytics_visitors.sql` (APLICADA em produção via conector):** `analytics_sessions.visitor_id uuid` + `is_returning` (trigger `analytics_mark_returning` before insert); funções `analytics_visitors` (new/returning/unknown), `analytics_consent` (accepted/essential/none por sessão, último `cookie_consent`), `analytics_journeys` recriada com `is_returning`
- **Tracker:** `visitor id` em `localStorage` `solida-v` (13 meses) enviado como `s.vid`; **não é criado e é apagado se o banner estiver em "Só o essencial"** (lê `solida-cookie-consent` direto e escuta `solida-consent-change` — strings duplicadas de `lib/analytics.ts` de propósito, para não criar ciclo de import). Clique genérico usa `innerText` (evita texto grudado). Endpoint grava `visitor_id`
- **`data-track` com nome legível em todos os CTAs** (é o que o ranking "Tudo que foi clicado" mostra): `WhatsAppCTAButton` → "Abrir Central: {origem}", `WhatsAppFloat`/`Header` (3) → "Abrir Central: Botão flutuante / Botão do cabeçalho / Ícone do cabeçalho (celular) / Menu (celular)", `WhatsAppModal` → "Central: {assunto}" e "Central: {assunto} › {região}", `WaTrackedLink` → "WhatsApp: {assunto} › {opção} ({origem})", `TrackedLink` → "Telefone: …/E-mail: …/Rede social: …/Mapa: …", `BlogGrid` → "Blog: Carregar mais artigos". **Padrão: todo CTA novo recebe `data-track` em português**
...
- **Slide de clientes REMOVIDO (17/09/2026):** a Sólida não tem autorização para usar as logos dos clientes. `prep_clients()`/`s_clients()` continuam no `build.py` fora da sequência, caso a autorização venha
- Capa: foto inteira em cima + bloco preto com o título (o título sobre a foto tampava o logo do baú)
- Slides: capa · conteúdo · quem somos · missão/visão/valores · números (32 anos / 365 cidades / 3 unidades) · serviços · passo a passo · segmentos · cobertura (diagrama SVG dos corredores) · principais praças · prazos e horários · unidades · contato (QR via `segno` para `/apresentacao`) · encerramento · 2 anexos de cidades

## Assinatura de e-mail (GIF animado da logo) — 17/09/2026

- **Pedido (briefing em `../Assinatura de Email/ASSINATURA DE EMAIL.pdf`):** assinatura oficial padronizada para todos os colaboradores, campos NOME/CARGO/CIDADE/FONE/RAMAL/WHATSAPP/EMAIL/SITE, logo em **GIF com efeito de aparecimento** (referência: vídeo da CW3 na mesma pasta — wipe esquerda→direita em loop), compatível com Gmail/Outlook/Apple Mail e legível no celular
- **Onde:** `../Assinatura de Email/` (fora do git): `build.py` (gera `out/logo-animada.gif` 380×148 px 2x/141 KB, `logo.png` estática, 5 ícones PNG 32 px a partir de SVG Lucide/Simple Icons renderizados com PyMuPDF e a foto da campanha; copia tudo + `gerador.html` para `public/assinatura/`), `assinatura.html` (template só com tabelas + estilos inline, Arial, `{{PLACEHOLDERS}}`), `gerador.html` (formulário → prévia → **Copiar assinatura** via ClipboardItem text/html, fallback execCommand; "Baixar .htm"; instruções por cliente), `COMO-INSTALAR.md`. **Regenerar/publicar: `python build.py`**; ao mudar o layout, manter `assinatura.html` e o `TEMPLATE` do `gerador.html` iguais
- **GIF (ciclo 5 s, loop):** logo completa parada 2,6 s → fade-out → branco 0,35 s → wipe esquerda→direita com borda suave (26 quadros/50 ms) → slogan "O nome já diz tudo!" em fade → repete. **1º quadro é a logo completa de propósito**: o Outlook clássico do Windows não anima GIF e mostra só ele. Fundo branco opaco; paleta única de 128 cores com branco forçado a #FFFFFF (a Pillow mapeava o fundo para 251/252/252 → borda cinza visível no e-mail). Slogan separado do resto por detecção da linha vazia do alpha (`load_logo`)
- **No ar (17/09/2026):** `public/assinatura/` → `https://www.solidatransporte.com.br/assinatura/{logo-animada.gif, logo.png, icone-cidade|fone|whatsapp|email|site.png, campanha-setembro-amarelo.jpg, gerador.html}`. **Gerador para a equipe: `https://www.solidatransporte.com.br/assinatura/`** (redirect em `next.config.ts` → `/assinatura/gerador.html`; header `X-Robots-Tag: noindex` + meta robots — uso interno, fora do Google e do sitemap)
- **Layout:** logo 190×74 (link p/ site) | divisor vermelho 2 px | nome 16 px bold `#E10600`, cargo `#6B7280`, linhas com ícone 16 px: cidade, fone (`tel:`) + "Ramal N", WhatsApp (`wa.me/55…`, ícone verde `#25D366`), e-mail (`mailto:`), site em vermelho bold. Largura ≈ 500 px sem campanha. Link do site leva `?utm_source=email&utm_medium=assinatura` (canal e-mail no dashboard)
- **Campanha Setembro Amarelo (17/09/2026, referência: assinatura da GPAX):** 3ª coluna à direita = foto 200×153 (`campanha-setembro-amarelo.jpg`, 2x, 20 KB) + título/texto/CVV 188 + link brcarga.com.br/telemedicina em HTML abaixo. A foto é só o recorte esquerdo (`CAMPAIGN_CROP = (0,0,1000,762)`) da arte `../Ficha Cadastral/novas proporções.png` — o texto da arte saiu por recorte, não por inpainting. Bloco entre marcadores `CAMPANHA`/`/CAMPANHA` nos dois templates; o gerador tem checkbox "Incluir campanha" (desmarcado → remove a coluna). Largura total com campanha ≈ 700 px. Passada a campanha: tirar o `checked` do checkbox ou apagar o bloco
- **24/09/2026 — escolha de campanha no gerador:** checkbox → 4 opções (Nenhuma / Setembro Amarelo / Outubro Rosa / Novembro Azul), pré-marcada pelo mês atual (set/out/nov; fora disso "Nenhuma"). `build.py`: `CAMPAIGNS = {slug: (arte, recorte)}` → `campanha-<slug>.jpg` (arte ausente é pulada com aviso). `gerador.html`: objeto `CAMPANHAS` com o bloco HTML de cada uma (Setembro = bloco do `TEMPLATE`; `null` = opção desabilitada "em breve"). Outubro Rosa: recorte `(1064,0,2064,762)` de `../Ficha Cadastral/Outubro Rosa.png` (foto à direita), título rosa `#C2185B` "Mais conscientização, mais vida" + texto da arte, sem link/telefone (a arte não tem). **Novembro Azul pendente: arte não recebida**
...
## Ficha cadastral (17/09/2026; GO/DF e Outubro Rosa em 23/09/2026)
...
- **Regra aprendida:** para a ficha, "modernizar" = minimalista e próximo da estrutura original (uma coluna, mesmos campos), não o visual pesado do deck. Dados no dict `UNITS` do script (copiados das fichas antigas, só com máscara nos números)
- **23/09/2026 — GO e DF no mesmo padrão:** `build-ficha.py` virou multi-unidade (`UNITS = {sp, go, df}`; `python build-ficha.py` gera as 3, `python build-ficha.py go df` só as indicadas; HTML sai em `ficha-{sp,go,df}.html`, o `ficha.html` antigo foi removido). PDFs: `Ficha Cadastral Sólida - Filial SP.pdf`, `... - Matriz GO.pdf`, `... - Filial DF.pdf`. Fontes: `ficha_cadastral_filial_go.pdf` e `Ficha cadastral DF.pdf` (imagens, lidas visualmente), conferidas com `lib/units.ts` e o deck. Decisões a confirmar com o cliente: (1) GO rotulada **Matriz Goiânia** (CNPJ /0001-30; a ficha antiga dizia "Filial GO"); (2) e-mail de expedição GO corrigido de `expdicao@soldiatransporte` (typo na ficha antiga) para `expedicao@solidatransporte.com.br`; (3) DF recebeu CEP 71225-525 vindo do site (ficha antiga sem CEP) e Inscrição municipal "—" (ficha antiga: "-")
- **23/09/2026 — campanhas:** `CAMPAIGNS = {setembro-amarelo, outubro-rosa}` no script; a arte do Outubro Rosa é `Outubro Rosa.png` (2064x762, mesma proporção). Por padrão o build gera 3 unidades x 2 campanhas = 6 PDFs; Setembro Amarelo mantém os nomes originais e Outubro Rosa recebe sufixo `- Outubro Rosa.pdf`. `--campanha outubro-rosa` gera só uma campanha. HTML sai em `ficha-{unidade}-{campanha}.html`; assets copiados para `assets/{campanha}.png`. Para uma campanha nova: adicionar a arte na pasta e uma entrada em `CAMPAIGNS`
...
- ~~Pendente validar com a Sólida: qual número atende o RJ~~ → resolvido em 23/09/2026: os mesmos WhatsApps da Central; sem opção RJ separada.
- **2026-08-27 — mapa da filial de Brasília:** `lib/units.ts` ganhou `BRASILIA_MAPS_QUERY` (endereço como o Google reconhece: STRC Trecho 02 Galpão 02 Conj. E Lote 03, 71225-525), `BRASILIA_MAPS_URL` (usado no `mapsUrl` da unidade) e `BRASILIA_MAP_EMBED` (embed por endereço, sem API key). `/contato` mostra os dois mapas em grid 2 col.; `/a-empresa` seção "Onde estamos" virou "Matriz em Goiânia e filial em Brasília" com 2 cards (mapa + endereço + link). Mesmo dia: Guarulhos ganhou `GUARULHOS_MAPS_QUERY/_URL/_MAP_EMBED` — as duas páginas agora exibem os 3 mapas (grid `md:2 / lg:3` col.).
- **2026-08-31 — mensagem única nos CTAs de WhatsApp (pedido do cliente):** TODOS os links wa.me do site passaram a abrir com "Olá, vim do site e gostaria de fazer uma cotação!" — constante `WA_DEFAULT_MESSAGE` em `lib/whatsapp.ts`, usada em todos os assuntos da Central (`WA_SUBJECTS`), nas 3 unidades (`lib/units.ts`) e nos 3 links do Footer. As mensagens específicas por assunto (coleta/rastreamento com campos ___) foram substituídas de propósito; se voltarem, estão no histórico desta data. **No mesmo dia o assunto "Financeiro e faturas" foi REMOVIDO da Central** (a pedido do cliente): saiu de `WA_SUBJECTS`, dos `SUBJECT_ICONS` (modal e /contato) e da description de /contato; o número segue em `WHATSAPP_NUMBERS.financeiro`; `SUBJECT_LABELS.financeiro` mantido em `analytics-types.ts` p/ dados antigos (mesmo padrão do juridico); redirect `pagamento-de-fatura` agora aponta p/ `/contato` sem âncora.
- **2026-08-27 — tracking dos mapas:** `maps_click` agora leva `unit`, `source` (contato | a-empresa) e `via` (`link` = clique em "Ver no mapa"/"Abrir no Google Maps"; `embed` = interagiu com o iframe). Novo `components/analytics/TrackedMapEmbed.tsx` (client; detecta interação no iframe por `window.blur` + `document.activeElement`, dispara `maps_click` via embed + `click` genérico `Mapa interativo: {unidade}` p/ o ranking) — usado nos 6 iframes de /contato e /a-empresa. Os 3 links "Abrir no Google Maps" de /a-empresa viraram `TrackedLink`. `TrackedLink` kind=maps exige `source`. Migração `0005_analytics_maps.sql` (aplicada no Supabase em 2026-08-27): `analytics_buttons` devolve `via` na coluna `option` p/ maps_click → dashboard mostra "Mapa: X" e "Mapa interativo: X" separados, pivô "onde foi clicado" ganha "Página A Empresa". GTM: DLV `via` + parâmetro na tag `maps_click` (reimportar container); TRACKING.md com a dimensão "Forma de abrir o mapa".
